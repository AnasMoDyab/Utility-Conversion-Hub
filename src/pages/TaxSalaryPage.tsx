...
const TaxSalaryPage: React.FC = () => {
  ...
  const effectivePct = ((tax + social) / gross) * 100;
  return (
    <Card variant="outlined" sx={{ maxWidth: 700 }}>
      <CardContent>
        <Typography variant="h5" gutterBottom>Lønn & Skatt</Typography>
        <Typography variant="body2" color="text.secondary" gutterBottom><b> </b> Grovt estimat av nettolønn etter skatt og sosiale avgifter (forenklede satser).</Typography>
        <Stack spacing={2}>
          ...
          <Typography>Skatt ({cfg.taxRate}%): {tax.toFixed(2)} | Sosiale avgifter ({cfg.socialRate}%): {social.toFixed(2)}</Typography>
          <Typography>Effektiv total fratrekksprosent: {effectivePct.toFixed(2)}%</Typography>
